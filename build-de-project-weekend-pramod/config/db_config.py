...
DB_USER = "postgres"
DB_PASS = "Mylearning@678"
DB_HOST = "localhost"
DB_PORT = "5432"
DB_NAME = "postgres"
password_escaped = urllib.parse.quote_plus(DB_PASS)

DATABASE_URL = f"postgresql+psycopg2://{DB_USER}:{password_escaped}@{DB_HOST}:{DB_PORT}/{DB_NAME}"
...
def set_spark_env(
    java_home: str = None,
    pyspark_python: str = None,
    pyspark_driver_python: str = None,
):
    """Set JAVA_HOME and PYSPARK_* env vars needed on this machine.

    Parameters:
        java_home: path to the Java installation. If not provided, existing
            JAVA_HOME is preserved and no instructor-specific default is used.
        pyspark_python: path to the Python executable used by PySpark workers.
            Defaults to the current interpreter.
        pyspark_driver_python: path to the Python executable used by the Spark
            driver. Defaults to the current interpreter.
    """
    if java_home:
        os.environ["JAVA_HOME"] = java_home
    elif "JAVA_HOME" in os.environ:
        print("[db_config] Using existing JAVA_HOME from environment.")
    else:
        print("[db_config] WARNING: JAVA_HOME is not set. Please provide java_home or set JAVA_HOME.")

    current_python = pyspark_python or pyspark_driver_python or os.path.abspath(os.path.realpath(__import__("sys").executable))
    os.environ["PYSPARK_PYTHON"] = pyspark_python or current_python
    os.environ["PYSPARK_DRIVER_PYTHON"] = pyspark_driver_python or current_python
